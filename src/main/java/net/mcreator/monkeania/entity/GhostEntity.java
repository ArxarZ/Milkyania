...
public class GhostEntity extends Monster {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(MonkeaniaModEntities.GHOST.get(), 20, 1, 4));
	}

	public GhostEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(MonkeaniaModEntities.GHOST.get(), world);
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new Goal() {
			...
			public boolean canContinueToUse() {
				return GhostEntity.this.getMoveControl().hasWanted() && GhostEntity.this.getTarget() != null
		...
		this.goalSelector.addGoal(2, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				Random random = GhostEntity.this.getRandom();
	...
	public static void init() {
		SpawnPlacements.register(MonkeaniaModEntities.GHOST.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				(entityType, world, reason, pos, random) -> (world.getDifficulty() != Difficulty.PEACEFUL
						&& Monster.isDarkEnoughToSpawn(world, pos, random) && Mob.checkMobSpawnRules(entityType, world, reason, pos, random)));
		DungeonHooks.addDungeonMob(MonkeaniaModEntities.GHOST.get(), 180);
	}
