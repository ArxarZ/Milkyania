...
public class ArachnaEntity extends Monster {
	...
	public ArachnaEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(MonkeaniaModEntities.ARACHNA.get(), world);
	}

	public ArachnaEntity(EntityType<ArachnaEntity> type, Level world) {
		super(type, world);
		xpReward = 2;
		setNoAi(false);
	}
	...
	public static void init() {
		SpawnPlacements.register(MonkeaniaModEntities.ARACHNA.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				(entityType, world, reason, pos, random) -> (world.getDifficulty() != Difficulty.PEACEFUL
						&& Monster.isDarkEnoughToSpawn(world, pos, random) && Mob.checkMobSpawnRules(entityType, world, reason, pos, random)));
		DungeonHooks.addDungeonMob(MonkeaniaModEntities.ARACHNA.get(), 180);
	}
